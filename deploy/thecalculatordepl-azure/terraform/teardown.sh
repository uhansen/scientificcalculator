...
require_cmds bash az gh curl sed mise
...
info "Destroying the Azure Terraform application stack"
(
  cd "${SCRIPT_DIR}"
...
ok "Azure application stack destroyed"

if [[ "${DESTROY_BACKEND:-false}" != "true" ]]; then
  warn "Backend storage account left in place. Set DESTROY_BACKEND=true to remove it as well."
  exit 0
fi

STATE_RESOURCE_GROUP_NAME="${STATE_RESOURCE_GROUP_NAME:-$(sed -n 's/^resource_group_name *= *\"\(.*\)\"$/\1/p' "${backend_file}")}"
STATE_STORAGE_ACCOUNT_NAME="${STATE_STORAGE_ACCOUNT_NAME:-$(sed -n 's/^storage_account_name *= *\"\(.*\)\"$/\1/p' "${backend_file}")}"
[[ -n "${STATE_STORAGE_ACCOUNT_NAME}" ]] || die "Set STATE_STORAGE_ACCOUNT_NAME to destroy the bootstrap storage account."
[[ -f "${bootstrap_dir}/terraform.tfstate" ]] || die "Missing ${bootstrap_dir}/terraform.tfstate required for bootstrap destroy."

info "Destroying the Azure Storage backend bootstrap stack"
(
  cd "${bootstrap_dir}"
...
ok "Azure backend bootstrap destroyed"
