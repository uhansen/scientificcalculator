...
output "backend_region" {
  description = "Region to use for backend configuration."
  value       = var.aws_region
}

output "backend_key_example" {
  description = "Example key path for the main Terraform state."
  value       = "deploy/thecalculatordepl-aws/terraform/terraform.tfstate"
}
...
output "backend_hcl_snippet" {
  ...
  value       = <<-EOT
  bucket       = "${aws_s3_bucket.terraform_state.id}"
  key          = "deploy/thecalculatordepl-aws/terraform/terraform.tfstate"
  region       = "${var.aws_region}"
  use_lockfile = true
  EOT
}
